...
class ParsingVCD:
            ...
            for var in instance.findall("./var")+instance.findall("./module/var"): # going all the vars in instances
                ...
                # If the var is in both <vcd> and <AST>
                    ...
                    if 'vartype' in self.ast_root.find(".//module[@name='"+submodname+"']/var[@name='"+var.attrib['name']+"']").attrib:
                        var_type = self.ast_root.find(".//module[@name='"+submodname+"']/var[@name='"+var.attrib['name']+"']").attrib["vartype"]
                        if var_type == "integer":
                            var.tag = 'integer'
                        elif var_type == "int":
                            var.tag = 'int'
                        elif var_type == "longint":
                            var.tag = 'longint'

                    if 'dir' in self.ast_root.find(".//module[@name='"+submodname+"']/var[@name='"+var.attrib['name']+"']").attrib:
                        dir_type = self.ast_root.find(".//module[@name='"+submodname+"']/var[@name='"+var.attrib['name']+"']").attrib["dir"]
                        if dir_type == "input":
                            var.attrib["dir"] = 'input'
    ...
    # Add attributes to sig_tree.
    # The attributes are the information of the type a signal. (EX: FF, wire)
    def attribute_modifying(self):
        module_sig_dict = {}
        # Find all <module>s in RTL-AST.
        for module in self.ast_root.findall(".//module"):
            submodule_name = module.attrib["name"]
            module_sig_dict[submodule_name] = {}
            # Find all <var> declarations in it.
            # Build a list of signals with them.
            var_list = [sig.attrib["name"] for sig in module.findall(".//var")]
            
            # Find all FF assignments. They should be under always@ blocks.
            for alwy in module.findall(".//always"):
                if alwy.find("./sentree") == None:
                    continue
                for assign in alwy.findall(".//assigndly") + alwy.findall(".//assign"):
                    # If a var is the left value of these kind of assignment, mark them as "FF".
                    left_value = assign.getchildren()[1]
                    if left_value.tag != "varref":
                        print("Error: left value should be a <varref>.")
                    else:
                        lval_name = left_value.attrib["name"]
                        if not lval_name in var_list:
                            print("Error: left value name is not in the var declaration list.")
                        else:
                            module_sig_dict[submodule_name][lval_name] = "FF"

        # Modified the matched signals in sig_tree.
        for submodname in module_sig_dict.keys():
            for ins in self.sig_tree_root.findall(".//instance[@submodname='"+submodname+"']"):
                for sig_name in module_sig_dict[submodname].keys():
                    sig = ins.find(".//var[@name='"+sig_name+"']")
                    if sig == None:
                        print("Error: Signal not found in sig_tree.")
                    elif module_sig_dict[submodname][sig_name] == "FF":
                        sig.attrib["FF"] = "true"

    def dump_sig_tree(self):
